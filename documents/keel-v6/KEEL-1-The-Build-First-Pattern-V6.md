# The Build-First Pattern (V6)

> "All good rules were written in blood."

> **What's new in v6:** v5's Principle 7 established the numbered decision log (`docs/decisions/`, one file per decision, five fields, recoverable). v6 extends the same discipline to the other two artifacts every project accumulates alongside decisions: **findings** (what you learned) and **the test plan** (what you verified). Both were previously left to "capture it in the repo somewhere." v6 makes them named, structured, standing documents with the same non-negotiable status as the decision log. Nothing in Principles 1–10 changed — they're paid for and stay as-is. Two principles were added: 11 and 12.
...
## The twelve principles (each learned the hard way)
...
### 11. Findings are a standing document, not a memory. *(new in v6)*

What you learn during a build — the dead end, the surprising constraint, the thing that worked better than expected, the corroboration technique that turned out unreliable — is as perishable as an unrecorded decision, and for the same reason: nobody can reconstruct it once they already know how the story ends. Keep a single findings log, dated entries, newest last. Each entry names the context (what you were doing), the finding (what you learned), and the implication (how it changes what you do next). This is not a bug tracker and not a decision — it's the residue of experience that would otherwise live only in whoever was in the room, and leave with them.

**PAID FOR IN BLOOD:** the same afternoon we caught a Planner asserting it could see a file in our repository that had never once been committed there (Principle 8's failure mode, in the wild, on our own project) — we had nowhere obvious to put that lesson. It didn't fit the decision log; it wasn't a decision. It would have evaporated as a "huh, weird" muttered once and forgotten, exactly the kind of thing that costs you a second time when someone repeats the same trust six months later. Writing it down as a finding is the only reason it won't.

### 12. The test plan is a first-class, tabular document — not an implication of "tests exist." *(new in v6)*

Principle 2 says tests guard deployment; it does not say anyone can see what those tests actually cover. A pass/fail badge tells you the suite is green — it does not tell you whether the thing that matters was ever checked. Keep one table: test ID, component/feature, test type, description, expected result, status, date, notes. Update it as you write tests, not retroactively. The table is what lets you — or anyone else — answer "is X actually tested?" in five seconds instead of reading the suite.

**PAID FOR IN BLOOD:** not yet paid on this project — this principle is being adopted deliberately, before the first line of test code, rather than retrofitted after a coverage gap bites. That is the point of the whole Keel discipline: some scars you get to avoid by reading someone else's.
