...
How to lay the Keel on a new project — the field manual for [The Build-First Pattern](./KEEL-1-The-Build-First-Pattern-V5.md).
...
- [ ] Create `docs/README.md` with two rules, plainly: (1) "Every design decision gets written down in `docs/decisions/` BEFORE the work it describes is finished." (2) "Every claim records the artefact it came from." (Rule 2 is Step 9 — write it now, practise it there.)
...
### Step 9 — Write down HOW you know, not just what you concluded

- [ ] In `docs/README.md`, add a second rule under the first: every claim records the artefact it came from — the log line, the query, the run link.
...
### Step 10 — Lock secrets out of the code (non-negotiable in a public repo)
...
- [ ] Add an automatic secret-scanner to your gate (Step 12) so a commit carrying a key is refused the same way a failing test is refused. Ask your Builder to wire this in — it is a few lines.
...
### Step 11 — Self-contained tests (do this one early — it makes all the rest easy)

- [ ] Set up the test framework so tests run in their own isolated world — no internet, no real services, no passwords required.
- [ ] Write one tiny throwaway test that checks something obvious (like 1 + 1 == 2), just so the machinery exists and runs.
...
### Step 12 — Build the gate (tests must pass before anything ships)
...
**PROOF:** comes in Step 13. Do not trust the gate yet. Prove it.
...
### Step 13 — PROVE the gate works (never skip this — it is the whole point)

- [ ] Deliberately write a test that FAILS. Push it on a branch.
- [ ] Watch the system REFUSE to deploy it. (If it ships anyway, your gate is fake — stop, fix Step 12.)
...
### Step 14 — Make the check BINDING (the gate stops the deploy; only this stops the human)
...
### Step 15 — Stop deploying by hand, forever
